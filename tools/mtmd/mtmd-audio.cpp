...
//
// mtmd_audio_preprocessor_ctc_conformer
//

void mtmd_audio_preprocessor_ctc_conformer::initialize() {
    cache.fill_sin_cos_table(hparams.audio_n_fft);
    cache.fill_hann_window(hparams.audio_window_len, true);
    cache.fill_mel_filterbank_matrix(
...
static void ctc_conformer_compute_deltas(const mtmd_audio_mel & in, mtmd_audio_mel & out) {
    out.n_mel     = in.n_mel;
    out.n_len     = in.n_len;
...
bool mtmd_audio_preprocessor_ctc_conformer::preprocess(const float *                 samples,
                                                        size_t                        n_samples,
                                                        std::vector<mtmd_audio_mel> & output) {
    if (n_samples == 0) {
        return false;
    ...
    ctc_conformer_compute_deltas(mel, deltas);
