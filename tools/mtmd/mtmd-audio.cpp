...
// REF: https://github.com/huggingface/transformers/blob/main/src/transformers/models/granite_speech/feature_extraction_granite_speech.py#L36
//
...
bool mtmd_audio_preprocessor_granite_speech::preprocess(const float *                 samples,
    ...
    params.hop_length       = hparams.audio_hop_len;
    params.sample_rate      = hparams.audio_sample_rate;
    params.center_padding   = true;           // match torchaudio default
    params.preemph          = 0.0f;           // no preemphasis
    params.use_natural_log  = false;          // use log10
    params.norm_per_feature = false;          // clamp/norm across all data

    GGML_ASSERT(!cache.sin_vals.empty());
    GGML_ASSERT(!cache.cos_vals.empty());
    GGML_ASSERT(!cache.filters.data.empty());

    // NOTE: Granite speech does custom normalization that is different, but
    //      equivalent to what is done in log_mel_spectrogram:
    //
    ...
    // Frame stacking: concatenate pairs of adjacent frames
    // Input: [n_mel=80, n_len] -> Output: [n_len/2, 160]
    int n_frames = mel_out.n_len;
    if (n_frames % 2 == 1) {
    ...
    output.push_back(std::move(final_out));
    return true;
}
