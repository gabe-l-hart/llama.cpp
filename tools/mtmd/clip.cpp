...
static std::unique_ptr<clip_graph> clip_get_graph_builder(clip_ctx * ctx, const clip_image_f32_batch & imgs,
    ...
    switch (ctx->proj_type()) {
        ...
        case PROJECTOR_TYPE_CTC_CONFORMER_FE:
            {
                builder = std::make_unique<clip_graph_ctc_conformer_fe>(ctx, img);
            } break;
...
struct clip_model_loader {
    ...
    void load_hparams(clip_model & model, clip_modality modality) {
        ...
        // other hparams
        {
            ...
            switch (model.proj_type) {
                ...
                case PROJECTOR_TYPE_CTC_CONFORMER_FE:
                    {
                        // front-end only (no learned encoder stack, see clip_graph_ctc_conformer_fe);
            ...
            if (is_vision) {
                ...
                // ctc-conformer-fe: n_mel_bins here is the post-delta-post-stack width (e.g. 320),
                // not a raw mel-bin count, so the <=256 cap doesn't apply to it
                if (hparams.n_mel_bins <= 0 ||
                    (fft_based && model.proj_type != PROJECTOR_TYPE_CTC_CONFORMER_FE && hparams.n_mel_bins > 256)) {
    ...
    static clip_image_f32_batch get_dummy_batch(clip_ctx & ctx_clip) {
        ...
        if (ctx_clip.model.modality == CLIP_MODALITY_VISION) {
            ...
            // so the [1, 256] bound only applies to FFT-based models. ctc-conformer-fe is
            // FFT-based, but n_mel_bins there is the post-delta-post-stack width (e.g. 320),
            // not a raw mel-bin count, so it's exempted from the cap too.
            const bool fft_based = ctx_clip.model.proj_type != PROJECTOR_TYPE_GEMMA4UA;
            if (hparams.n_mel_bins <= 0 ||
                (fft_based && ctx_clip.model.proj_type != PROJECTOR_TYPE_CTC_CONFORMER_FE && hparams.n_mel_bins > 256)) {
