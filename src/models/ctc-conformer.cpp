...
// granite-speech-ctc: a CTC (Connectionist Temporal Classification) acoustic model. Non-causal,
...
// mmproj, see conversion/granite.py's GraniteSpeechCTCFrontendMmprojModel) rather than token
...
void llama_model_ctc_conformer::load_arch_hparams(llama_model_loader & ml) {
    ...
    std::vector<int32_t> subsample_layers;
    ml.get_arr(LLM_KV_CTC_SUBSAMPLE_LAYERS, subsample_layers, false);
    for (int32_t il : subsample_layers) {
        hparams.is_subsample_impl[il] = 1;
    }
}

void llama_model_ctc_conformer::load_arch_tensors(llama_model_loader &) {
    LLAMA_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> llama_model_ctc_conformer::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

llama_model_ctc_conformer::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // conv module
        {
            ...
            const bool subsample = hparams.is_subsample(il);
            ...
            if (subsample) {
                ...
                x = ggml_view_3d(ctx0, x, x->ne[0], half_len, x->ne[2], x->nb[1] * 2, x->nb[2], 0);
                x = ggml_cont(ctx0, x);
            }
