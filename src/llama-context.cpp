...
llama_context::llama_context(
    ...
    if (!hparams.vocab_only) {
        ...
        // If the model is hybrid, also initialize the hybrid cache
        const bool recurrent = llama_model_is_recurrent(&model);
        const bool hybrid = llama_model_is_hybrid(&model);
        if (hybrid) {
            // TODO: Should this create recurrent memory differently somehow?
            kv_hybrid.reset(static_cast<llama_kv_cache_unified *>(model.create_memory()));
        }
        ...
        if (!kv_self->init(model, cparams, type_k, type_v, kv_size, cparams.offload_kqv, recurrent && !hybrid)) {
        ...
        if (kv_size_hybrid > 0 && !kv_hybrid->init(model, cparams, type_k, type_v, kv_size_hybrid, cparams.offload_kqv, true)) {
            throw std::runtime_error("failed to initialize hybrid recurrent cache");
        }

        {
            const size_t memory_size_k = kv_hybrid->size_k_bytes();
            const size_t memory_size_v = kv_hybrid->size_v_bytes();
...
llama_kv_cache * llama_context::get_kv_hybrid() {
    return kv_hybrid.get();
}

const llama_kv_cache * llama_context::get_kv_hybrid() const {
    return kv_hybrid.get();
}
...
public:
    llm_graph_input_k_shift(const llama_kv_cache_unified * kv) : kv(kv) {}
    ...
    const llama_kv_cache_unified * kv;
};
...
llm_graph_result_ptr llama_context::build_kv_shift(
        ggml_context * ctx0,
        ggml_cgraph * gf,
        const std::unique_ptr<llama_kv_cache_unified> & kv) const {
  ...
  //const auto & n_embd_head_v = hparams.n_embd_head_v;
    ...
    auto inp = std::make_unique<llm_graph_input_k_shift>(kv.get());
...
llm_graph_result_ptr llama_context::build_kv_defrag(
        ggml_context * ctx0,
        ggml_cgraph * gf,
        const std::unique_ptr<llama_kv_cache_unified> & kv) const {
    auto res = std::make_unique<llm_graph_result>();
...
void llama_context::kv_hybrid_update() {
    if (kv_hybrid) {
        kv_update(kv_hybrid);
    }
}

void llama_context::kv_update(const std::unique_ptr<llama_kv_cache_unified> & kv) {
    ...
    if (kv->has_shift) {
        ...
        if (model.hparams.rope_type != LLAMA_ROPE_TYPE_NONE) {
            ...
            auto res = build_kv_shift(ctx_compute.get(), gf, kv);
    ...
    if (kv->do_defrag) {
        ...
        if (kv->defrag_prepare(graph_max_nodes())) {
            ...
            auto res = build_kv_defrag(ctx_compute.get(), gf, kv);
...
int llama_context::decode(llama_batch & inp_batch) {
    ...
    llama_kv_cache_guard kv_hybrid_guard(kv_hybrid.get());
    ...
    sbatch.from_batch(batch, n_embd,
            /* simple_split */ !(kv_self->recurrent || kv_hybrid),
    ...
    while (sbatch.n_tokens > 0) {
        ...
        if (kv_self->recurrent || kv_hybrid) {
            if (embd_pooled) {
        ...
        // find KV slot
        {
            ...
            if (kv_hybrid && !kv_hybrid->find_slot(ubatch)) {
                LLAMA_LOG_WARN("%s: failed to find hybrid KV cache slot for ubatch of size %d\n", __func__, ubatch.n_tokens);
...
size_t llama_context::state_write_data(llama_io_write_i & io) {
    ...
    if (kv_hybrid) {
        LLAMA_LOG_DEBUG("%s: - writing KV hybrid\n", __func__);
        kv_hybrid->state_write(io);
    }
...
size_t llama_context::state_read_data(llama_io_read_i & io) {
    ...
    if (kv_hybrid) {
        LLAMA_LOG_DEBUG("%s: - reading KV hybrid\n", __func__);
        kv_hybrid->state_read(io);
    }
...
size_t llama_context::state_seq_write_data(llama_io_write_i & io, llama_seq_id seq_id) {
    ...
    if (kv_hybrid) {
        kv_hybrid->state_write(io, seq_id);
    }
...
size_t llama_context::state_seq_read_data(llama_io_read_i & io, llama_seq_id seq_id) {
    ...
    if (kv_hybrid) {
        kv_hybrid->state_read(io, seq_id);
    }
...
llama_kv_cache * llama_get_kv_hybrid(llama_context * ctx) {
    return ctx->get_kv_hybrid();
}
...
void llama_kv_hybrid_update(llama_context * ctx) {
    ctx->kv_hybrid_update();
}
