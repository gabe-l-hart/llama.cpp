...
llama_context::llama_context(
    ...
    // decoder-less encoder-only architectures (e.g. CTC transcription) have no incremental
    // generation to split across ubatches - their one-shot encode() pass must fit the whole
    // input in a single ubatch (GGML_ASSERT(cparams.n_ubatch >= n_tokens) in encode()). raise
    // the physical/micro batch size ceiling to n_ctx by default instead of the (much smaller)
    // generation-oriented default, so a typical input "just works" without the caller needing
    // to know this is an encoder-only architecture and pass -b/-ub explicitly.
...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    // most encoder graphs produce exactly one output per input token, but an architecture
    // is free to change the sequence length internally (e.g. time subsampling) - read the
    // actual length off whichever output tensor the graph produced instead of assuming it
    // matches n_tokens; output_reserve(n_tokens) above already sized the buffers to the
    // upper bound, since a graph can only ever shrink the sequence, not grow it
