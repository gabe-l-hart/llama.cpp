...
// greedy CTC decode: argmax per output row, collapse consecutive repeats, drop blank (id 0),
// detokenize the survivors. `n_outputs` should come from llama_n_outputs(ctx) - a CTC encoder
// (e.g. a conformer with internal time subsampling) may produce fewer output rows than input
// tokens, and llama_get_logits_ith() does not itself bound-check against that reduced count.
